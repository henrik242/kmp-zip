        ...
        ZipInputStream(input, password?.encodeToByteArray()).use { zis ->
            ...
            while (true) {
                ...
                if (entry.isDirectory) {
                    ...
                    SinkOutputStream(sink(safe).buffered()).use { fos ->
...
private suspend fun addRecursive(
    ...
    val md = fs.metadataOrNull(file) ?: error("Cannot read metadata for $file")
    ...
    if (md.isDirectory) {
        ...
        zos.putNextEntry(ZipEntry(entryName))
        SourceInputStream(fs.source(file).buffered()).use { fis ->
